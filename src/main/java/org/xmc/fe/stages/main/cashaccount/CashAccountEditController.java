...
public class CashAccountEditController implements IValidationController {
    ...
    public void initialize() {
        ...
        cashAccountCurrencyComboBox.setConverter(GenericItemToStringConverter.getInstance(currency -> String.format("[%s] %s", currency.getCurrencyCode(), currency.getDisplayName())));
    ...
    public Multimap<IValidationComponent, String> validate() {
        ...
        if (bankComboBox.getSelectionModel().getSelectedItem() == null && StringUtils.isBlank(bankNameTextfield.getText())) {
